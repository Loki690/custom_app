	...
	bind_events() {
		...
		this.$payment_modes.on("click", ".mode-of-payment", function (e) {
			...
			if (mode_clicked.hasClass("border-primary")) {
				...
				me.selected_mode && me.selected_mode.$input.get(1).focus();
			...
			if (mode === "credit_card" || mode === 'debit_card') {
				...
				this[`bank_name_control_${mode}`] = frappe.ui.form.make_control({
					df: {
						...
						onchange: function () {
							frappe.model.set_value(p.doctype, p.name, "bank_name", this.value);
						},
					},
				...
				this[`name_on_card_control_${mode}`] = frappe.ui.form.make_control({
					df: {
						...
						onchange: function () {
							frappe.model.set_value(p.doctype, p.name, "name_on_card", this.value);
						},
					},
				...
				this[`card_name_control_${mode}`] = frappe.ui.form.make_control({
					df: {
						label: "Card Number",
						fieldtype: "Data",
						placeholder: 'Card Number',
						onchange: function () {
							frappe.model.set_value(p.doctype, p.name, "card_number", this.value);
						},
					},
				...
				this[`expiry_date_control_${mode}`] = frappe.ui.form.make_control({
					df: {
						label: "Expiry Date",
						fieldtype: "Date",
						placeholder: 'Expiry Date',
						onchange: function () {
							frappe.model.set_value(p.doctype, p.name, "card_expiry_date", this.value);
						},
					},
				...
				this[`reference_number_control_${mode}`] = frappe.ui.form.make_control({
					df: {
