...
class CustomStockBalanceReport:
		...
		item_warehouse_map[group_by_key] = frappe._dict(
			{
				...
				"item_name": entry.item_name,
   				"custom_principal": entry.custom_principal,
	...
	def apply_items_filters(self, query, item_table) -> str:
		# Filter by Item Group and its descendants
		if item_group := self.filters.get("item_group"):
			children = get_descendants_of("Item Group", item_group, ignore_permissions=True)
			query = query.where(item_table.item_group.isin([*children, item_group]))

		# Filter by other fields: item_code, brand, and custom_principal
		for field in ["item_code", "brand", "custom_principal"]:
