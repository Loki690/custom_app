erpnext.PointOfSale.ItemCart = class {
	constructor({ wrapper, events, settings }) {
		...
		this.allowed_doctors_groups = settings.doctors_groups;
			...
			`<div class="cart-container">
				<div class="abs-cart-container">
					...
					<div class="cart-header">
						...
						<div class="qty-header" style = "margin-left: 50px;">${__("Expiry")}</div>
	...
	make_doctors_selector() {
		...
		`);
		const me = this;
		const allowed_doctors_group = this.allowed_doctors_groups || [];
		let filters = {};
		if (allowed_doctors_group.length) {
			filters = {
				doctors_group: ["in", allowed_doctors_group],
			};
		}
			...
			df: {
				...
				onchange: function () {
					if (this.value) {
						const frm = me.events.get_frm();
						frappe.dom.freeze();
						frappe.model.set_value(frm.doc.doctype, frm.doc.name, "Doctors", this.value);
						frm.script_manager.trigger("Doctors", frm.doc.doctype, frm.doc.name).then(() => {
							frappe.run_serially([
								// () => me.fetch_doctors_details(this.value),
								// () => me.events.doctors_details_updated(me.customer_info),
		...
		this.doctors_field.toggle_label(false);
	}
	...
	fetch_doctor_details(doctors) {
		if (doctors) {
			return new Promise((resolve) => {
				frappe.db
					.get_value("Doctor", doctor, ["first_name", "last_name", "prc_number"])
					.then(({ message }) => {
						// const { loyalty_program } = message;
						// if loyalty program then fetch loyalty points too
	...
	update_doctors_section() {
		const me = this;
		const { doctors, first_name = "", last_name = "", prc_number } = this.doctors_info || {};
				...
				`<div class="doctors-details">
					<div class="doctors-display">
						<div class="doctors-name-desc">
							<div class="doctors-name">${doctors}</div>
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_expiry_html() {

			if (item_data.transaction_date) {
				if (item_data.transaction_date.indexOf("<div>") != -1) {
					try {
						item_data.transaction_date = $(item_data.transaction_date).text();
					} catch (error) {
						item_data.transaction_date = item_data.transaction_date
							.replace(/<div>/g, " ")
							.replace(/<\/div>/g, " ")
							.replace(/ +/g, " ");
					}
				}
				item_data.transaction_date = frappe.ellipsis(item_data.transaction_date, 45);
				return `<div class="item-desc" style ="margin-left: 20px; font-weight: bold;" >${item_data.transaction_date}</div>`;
			}
			return ``;
